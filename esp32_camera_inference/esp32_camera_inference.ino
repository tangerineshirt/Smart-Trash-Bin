...
void setup() {
  ...
  setupEspNow();  // Inisialisasi ESP-NOW
...
void loop() {
  ...
  for (uint16_t i = 0; i < EI_CLASSIFIER_LABEL_COUNT; i++) {
    ...
    if (result.classification[i].value > maxVal) {
      ...
      topLabel = ei_classifier_inferencing_categories[i];
    }
  }
#endif
  // Kirim label terbaik via ESP-NOW
  strncpy(myData.label, topLabel, sizeof(myData.label));
  esp_err_t resultSend = esp_now_send(esp32s3Address, (uint8_t *)&myData, sizeof(myData));
  if (resultSend != ESP_OK) {
    Serial.println("Gagal kirim ESP-NOW");
  } else {
    Serial.printf("Label \"%s\" berhasil dikirim ke ESP32-S3\n", myData.label);
  }
